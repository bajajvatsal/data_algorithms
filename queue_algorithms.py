...
class Queue:
    ...
    def is_empty(self):
        if self.length == self.rear:
            print("Queue Underflow! No operations can be done")
    ...
    def enqueue(self, val):
        if self.front == self.limit:
            print("Queue Overflow! No operations can be done")
        else:
...
if __name__ == '__main__':
    ...
    if length_array == limit_array:
        print("No space available operations cant be done. Please re-run the program")
        sys.exit()
    elif length_array > limit_array:
        print("Length of queue must be lesser than the limit . Please re-run the program")
        sys.exit()
